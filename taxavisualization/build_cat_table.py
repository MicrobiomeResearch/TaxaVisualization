...
def identify_groups(mapping, category):
    ...
    """
     ...
     # Convers the two-dimensional dictionary to a list
    items = [i for i in mapping.itervalues()]
    keys = items[0].keys()

    # Preforms a sanity check on category
    if category not in keys:
        raise ValueError('%s is not an avaliable category.' % category)

    # Gets the list of possible values
    groups = set()
    for item in items:
        groups = groups.union([item[category]])
...
def build_sub_table(data, sample_ids, target_ids):
    ...
    INPUTS:
        ...
        target_ids -- the list of ids ot be included in the output table

    OUTPUTS:
        new_data -- a subset of the data specified by target_ids
    """
...
def sort_samples(data, sample_ids, sort_key=None, reverse=False):
    ...
    INPUTS:
        ...
        sort_key -- A numeric value or indicating the location in the
                    categories to use for sorting the data, or a None value.
                    None indicates data should be sorted by the sample ids.
                    Common Categories must be provided to sort numerically.
                    DEFAULT: None
...
def sort_categories(data, categories, first_cat, sort_method='ALPHA_WRAP',
    ...
    INPUTS:
        ...
        sort_method -- a string describing how the data should be sorted.
                    "ALPHA" says start at the first_cat value, and follow with
                        ...
                        category. For example, if the groups are DOG, CAT,
                        BIRD, SMALL, and FISH, and the first_cat is DOG, the
                        order would be DOG, BIRD, CAT, FISH, SMALL.
    ...
    if sort_method == 'ALPHA' or sort_method == 'RETAIN':
        ...
        for cat in category_order:
            order = [get_category_position(categories, cat) for
                     cat in category_order]
